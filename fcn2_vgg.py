...
class FCN2VGG:
    def __init__(self, batchSize, statsFile, enableTensorboardVisualization, vgg16_npy_path=None):
    ...
    def build(self, rgb, keepProbability, num_classes=2, random_init_fc8=True, debug=False):
        ...
        if self.enableTensorboardVisualization:
            tf.image_summary('Original', rgb, max_images=10)
            tf.image_summary('Foreground Prob', tf.expand_dims(self.probabilities[:,:,:,1] * 255, -1), max_images=10)
    ...
    def _conv_layer(self, bottom, name):
        with tf.variable_scope(name) as scope:
            ...
            if self.alpha is None:
                ...
                relu = self.leakyRelu(bias, name=name + '_lrelu', alpha=self.alpha)
            else:
                relu = self.parametricRelu(bias, name=name + '_prelu')
    ...
    def leakyRelu(self, features, name, alpha=0):
        return tf.maximum(alpha * features, features, name=name)

    def parametricRelu(self, features, name):
        param = tf.Variable(0.0, name=name + '_param')
        return tf.maximum(param * features, features, name=name)
